...
public class Item_RFID_Sensor_ASRS : Item_Parent{
	...
	private void HandleRackTriggerEnter(Collider other){
		...
		// Debug.Log($"Item_RFID_Sensor_ASRS: Trigger entered by '{other.name}'.", other.gameObject);

		if (ConveyorBeltReference == null)
		{
			Debug.LogError("Item_RFID_Sensor_ASRS: Conveyor reference is not assigned.", this);
			return;
		}
		...
		{
			// Debug.Log("Item_RFID_Sensor_ASRS: Entering object is not part of a slotted table.", other.gameObject);
			return;
		}

		if (tablesInTrigger.Contains(slottedTable))
		{
			// Debug.Log($"Item_RFID_Sensor_ASRS: Table '{slottedTable.TableID}' is already in trigger set.", slottedTable);
			return;
		}
		...
		if (IsProcessing)
		{
			// Debug.Log("Item_RFID_Sensor_ASRS: Already processing a table.", this);
			return;
		}

		// Debug.Log($"Item_RFID_Sensor_ASRS: Table '{slottedTable.TableID}' entered with task '{slottedTable.task}'.", slottedTable);

		if (slottedTable.task == RACK_TASK.NONE)
		{
			// Debug.Log($"Item_RFID_Sensor_ASRS: Table '{slottedTable.TableID}' has no active task. Ignoring.", slottedTable);
			return;
		}
		...
		{
			// Debug.Log($"Item_RFID_Sensor_ASRS: Lesson hasn't reached '{conveyorReturnedActionId}' yet — letting '{slottedTable.TableID}' pass through untouched.", slottedTable);
			return;
		}
		...
		if (spline == null)
		{
			Debug.LogWarning($"Item_RFID_Sensor_ASRS: Table '{slottedTable.TableID}' is missing Spline_Animate.", slottedTable);
			return;
		}

		if (slottedTable.task == RACK_TASK.INSERT && !rack.NeedsRackReturn(slottedTable))
		{
			// Debug.Log($"Item_RFID_Sensor_ASRS: Table '{slottedTable.TableID}' slot is already occupied. Skipping insert.", slottedTable);
			return;
		}
	...
	private IEnumerator ProcessRackTask(Item_Slotted_Table table, Spline_Animate scannedSpline)
	{
		...
		switch (table.task)
		{
			...
			case RACK_TASK.INSERT:
			{
				...
				// Debug.Log($"Item_RFID_Sensor_ASRS: Inserted table '{table.TableID}' into rack.", table);
				break;
			}
			...
			default:
			{
				// Debug.Log($"Item_RFID_Sensor_ASRS: No processing needed for table '{table.TableID}'.", table);
				break;
			}
		}
	...
	private IEnumerator RetrieveAndReturnToConveyor(string tableId, Vector3 dropPosition, Quaternion dropRotation, float returnOffset)
	{
		...
		if (retrieved == null)
		{
			Debug.LogWarning($"Item_RFID_Sensor_ASRS: No stored table found for '{tableId}'.", this);
			ClearProcessingState();
			yield break;
		}
	...
	public void OnTriggerExit(Collider other)
	{
		...
		if (tablesInTrigger.Contains(slottedTable))
		{
			...
			// Debug.Log($"Item_RFID_Sensor_ASRS: Table '{slottedTable.TableID}' exited scanner.", slottedTable);
		}
	}
